...
import { formatCurrency, formatDate, formatDateTime, toFixed2 } from '../text/formatting';

export function renderInvoicePdf(document: PdfDocument, data: InvoicePdfData, branding: PdfBranding): void {
  ...
  y = drawKeyValue(document, 'Invoice Date:', formatDateTime(data.createdAt), y);
  ...
  y = drawTable(
    document,
    [
      { header: 'Item', width: 180, render: (row) => row.sparePartName ?? row.sparePartId },
      { header: 'Qty', width: 45, align: 'center', render: (row) => String(row.quantity) },
      { header: 'Unit Price', width: 90, align: 'right', render: (row) => formatCurrency(row.unitPrice, row.currency) },
      { header: 'Total', width: 90, align: 'right', render: (row) => formatCurrency(row.totalPrice, row.currency) },
    ],
    data.items,
    y,
  );
  ...
  y = drawKeyValue(document, 'Subtotal:', formatCurrency(data.totalAmount, data.totalCurrency), y, 150);
  y = drawKeyValue(document, 'Paid Amount:', formatCurrency(data.paidAmount, data.totalCurrency), y, 150);
  y = drawKeyValue(document, 'Remaining Balance:', formatCurrency(data.remainingAmount, data.totalCurrency), y, 150);
  ...
  y = drawTable(
    ...
    [
      ...
      { header: 'Amount', width: 100, align: 'right', render: (row) => formatCurrency(row.amount, row.currency) },
      { header: 'Converted', width: 100, align: 'right', render: (row) => toFixed2(row.convertedAmount) },
  ...
  if (data.warrantyPeriod || data.needsCenterMaintenance || data.notes) {
    ...
    if (data.warrantyPeriod) {
      y = drawText(document, `Warranty: ${data.warrantyPeriod}`, document.page.margins.left, y, {
        fontSize: 9,
        height: 6,
      });
    }
    if (data.needsCenterMaintenance) {
      y = drawText(document, `Center maintenance: ${data.needsCenterMaintenance}`, document.page.margins.left, y, {
        fontSize: 9,
        height: 6,
      });
    }
    if (data.notes) {
      y = drawText(document, `Notes: ${data.notes}`, document.page.margins.left, y, {
        fontSize: 9,
        height: 6,
      });
    }
